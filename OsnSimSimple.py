...
def readGraph(gFile, limit):
   	...
   	for line in graph:
   	   ...
   	   connections.append(array.array('l', idi[2:]))
   	   if len(idi) > 2:
...
print "index space = ", length
print p1a.count(True)/float(N), p2a.count(True)/float(N)
print p1a.count(True), p2a.count(True)


seedRand(users, socialGraph, p1a, p1b, float(sys.argv[6]))
seedRand(users, socialGraph, p2a, p2b, float(sys.argv[7]))
